    ...
    public static Element[][] dKGen(int d, PARS pars, Element[] S_B, Element[] P_A) {
        ...
        */
        ...
        for (int i = 0; i < S_B.length; i++) {
            ...
            Dd[0][i] = pars.get_g().duplicate().powZn((k_i_1.mul(pars.getTheta()[0].mul(pars.getTheta()[1]))).add(k_i_2.mul(pars.getTheta()[3].mul(pars.getTheta()[4])))).getImmutable();
            ...
            Dd[3][i] = Utils.T(S_B[i], pars).mul(k_i_2.negate().mul(pars.getTheta()[3]));
            Dd[4][i] = Utils.T(S_B[i], pars).mul(k_i_2.negate().mul(pars.getTheta()[2]));
        }
        for (int i = 0; i < P_A.length; i++) {
            Element r_i_prime_1 = pars.getZp().newRandomElement().getImmutable(), r_i_prime_2 = pars.getZp().newRandomElement().getImmutable();
            Dd_prime[0][i] = pars.get_g().duplicate().powZn((r_i_prime_1.mul(pars.getTheta()[0].mul(pars.getTheta()[1]))).add(r_i_prime_2.mul(pars.getTheta()[3].mul(pars.getTheta()[4])))).getImmutable();
            ...
            Dd_prime[3][i] = Utils.H(P_A[i], pars).mul(r_i_prime_2.negate().mul(pars.getTheta()[3]));
            Dd_prime[4][i] = Utils.H(P_A[i], pars).mul(r_i_prime_2.negate().mul(pars.getTheta()[2]));
        }
